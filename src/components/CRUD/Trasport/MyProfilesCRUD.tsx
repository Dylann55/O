...
import { sortItems } from '@/utils/SearchFilter';
...
const MyProfilesCRUD: React.FC<MyProfilesCRUDProps> = ({ urls, title, subtitle, role }) => {
    ...
    const filterItems = (items: Item[]) => {
        ...
        return items.filter((item) => {
            const propValue = item.organization[searchType];
            ...
            <div className='min-h-screen mx-6 my-2'>
                ...
                <SearchComponent searchTerm={searchTerm} setSearchTerm={setSearchTerm}>
                    ...
                    >
                        ...
                        <option value="name">Nombre de la Organizacion</option>
                    </select>
                </SearchComponent>
                        ...
                        <tbody className="divide-y divide-gray-200 text-center">
                            {getCurrentPageItems().map(item => (
                                ...
                                <tr key={item.organization.organizationID}>
                                    ...
                                    {isChofer && (
                                        <th className="text-center whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                                            <Link href={`/Transport/Routes/${item.organization.organizationID}?name=${encodeURIComponent(item.organization.name)}`}>
                                                <CustomButton
                                                    type="button"
                                                    color="indigo"
                                                    padding_x="4"
